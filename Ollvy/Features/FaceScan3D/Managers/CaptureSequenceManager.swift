...
public class CaptureSequenceManager: ObservableObject {
    ...
    public func startCaptureSequence() {
        ...
        // CRITICAL: Set isGuidanceActive IMMEDIATELY (not deferred)
        // This flag controls whether ARFrames are captured in ARFaceTrackingViewController
        // If deferred, the first several frames won't have frame data, blocking countdown
        self.isGuidanceActive = true

        // Defer other @Published property updates to avoid "Publishing changes from within view updates"
    ...
    public func capturePose(
        ...
        // CRITICAL FIX: Set these flags IMMEDIATELY to prevent race conditions
        // - isCaptureInProgress prevents checkGuidancePoseAndCapture from starting another countdown
        // - shouldTriggerCapture = false prevents re-triggering from observers
    ...
    public func onMultiFrameCaptureStarted() {
        ...
        // This flag is checked in checkGuidancePoseAndCapture to prevent starting another countdown
        self.isCaptureInProgress = true
    }
