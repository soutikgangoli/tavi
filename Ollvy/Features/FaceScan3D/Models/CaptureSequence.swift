...
public class CaptureSequence: Codable {
    ...
    /// Release mesh captures to free memory after merging is complete
    /// MEMORY OPTIMIZATION: Call this after mesh merge is complete to free ~10-30MB per capture
    public func releaseCaptures() {
        let captureCount = captures.count
        let vertexCount = captures.reduce(0) { $0 + $1.vertices.count }
        let estimatedMB = Double(vertexCount * MemoryLayout<Vector3>.stride * 3) / 1_048_576.0

        captures.removeAll()

        AppLogger.memory.info("🧹 Released \(captureCount) mesh captures (~\(String(format: "%.1f", estimatedMB)) MB)")
    }

    /// Release texture samples to free memory after baking is complete
    /// MEMORY OPTIMIZATION: Call this after texture bake is complete to free texture data
    public func releaseTextureSamples() {
        let sampleCount = textureSamples.count
        let estimatedMB = textureSamples.reduce(0.0) { $0 + Double($1.textureImageData.count) / 1_048_576.0 }

        textureSamples.removeAll()

        AppLogger.memory.info("🧹 Released \(sampleCount) texture samples (~\(String(format: "%.1f", estimatedMB)) MB)")
    }
}
...
public struct MergedFaceMesh: Codable {
    ...
    public func toOBJ() -> String {
        var obj = "# Ollvy Merged Face Mesh\n"
