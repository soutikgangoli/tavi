...
public class FaceScan3DViewModel: ObservableObject {
    ...
    public func bakeTextureFromSequence() async -> TextureBakeResult? {
        ...
        let result = await self.processingPipeline.bakeUnifiedTexture(
            from: merged,
            samples: sequence.textureSamples
        )

        // MEMORY OPTIMIZATION: Release texture samples after baking is complete
        // The baked texture contains all the color data we need - original samples are no longer needed
        if result != nil {
            sequence.releaseTextureSamples()
        }
    ...
    public func exportToOBJ() -> String? {
        ...
        var obj = "# Ollvy Face Mesh Export\n"
    ...
    private func setupPropertyForwarding() {
        ...
        // FIXED: Defer objectWillChange.send() using asyncAfter to avoid
