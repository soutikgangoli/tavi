...
public final class PerformanceAnalyzer {
    ...
    private let logger = Logger(subsystem: "com.tavi.app", category: "PerformanceAnalyzer")
    private let queue = DispatchQueue(label: "com.tavi.performance", qos: .utility)
    ...
    public func analyzeSequentialOperations() -> [SequentialOperation] {
        ...
        for (baseName, group) in operationGroups where group.count > 1 {
            ...
            let avgDuration = totalDuration / Double(group.count)
